...
class CLITestCase(BaseTestCase):
    ...
    def test_init_command(self):
        ...
        self.assertIn(
            'Creating the temporary administrator account...', result.output)
    ...
    def test_init_command_with_update(self):
        ...
        self.assertIn(
            'The administrator already exists, updating...', result.output)
        self.assertNotIn(
            'Creating the temporary administrator account...', result.output)
        self.assertEqual(Admin.query.count(), 1)
    ...
    def test_forge_command_with_count(self):
        result = self.runner.invoke(
            args=['forge', '--category', '5', '--post', '20', '--comment', '100'])
        self.assertEqual(Admin.query.count(), 1)
        self.assertIn('Generating the administrator...', result.output)
